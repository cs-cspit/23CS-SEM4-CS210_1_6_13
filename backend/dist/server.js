...
function generateAISuggestions(code, language) {
    const suggestions = [];
...
for (const item of items) {
  ...
  // Handle error appropriately
    ...
    const hasPoorNaming = lines.some(line => line.match(/\b(x|y|z|a|b|c|temp|var)\b/) &&
        !line.includes('//') &&
        !line.includes('*') &&
        !line.includes('+'));
    if (hasPoorNaming) {
        suggestions.push({
            ...
            lineNumbers: lines.map((line, index) => line.match(/\b(x|y|z|a|b|c|temp|var)\b/) ? index + 1 : -1).filter(n => n !== -1),
            severity: 'info',
...
const onlineUsers = activeUsers;`
    ...
    const hasDuplication = lines.some((line, index) => lines.slice(index + 1).some(nextLine => line.trim() === nextLine.trim() &&
        line.trim() !== '' &&
        !line.includes('}') &&
        !line.includes('{')));
    if (hasDuplication) {
...
users.forEach(processUser);`
    ...
    if (code.includes('setInterval') && !code.includes('clearInterval')) {
        suggestions.push({
            ...
            lineNumbers: lines.map((line, index) => line.includes('setInterval') ? index + 1 : -1).filter(n => n !== -1),
            severity: 'critical',
...
// Add this function after analyzeComplexity
function trackMemoryUsage(process) {
...
async function executeCode(code, language, input = '') {
    ...
    try {
        ...
        if (language === 'java') {
            try {
                await new Promise((resolve, reject) => {
                    const checkJava = (0, child_process_1.spawn)('java', ['-version']);
                    checkJava.on('error', () => reject(new Error('Java is not installed. Please install Java to run Java code.')));
                    checkJava.on('close', (code) => {
                        if (code === 0)
                            resolve(null);
                        else
                            reject(new Error('Java is not installed. Please install Java to run Java code.'));
                    });
                });
            }
            catch (error) {
        ...
        if (language === 'cpp') {
            try {
                await new Promise((resolve, reject) => {
                    const checkGpp = (0, child_process_1.spawn)('g++', ['--version']);
                    checkGpp.on('error', () => reject(new Error('C++ compiler (g++) is not installed. Please install g++ to run C++ code.')));
                    checkGpp.on('close', (code) => {
                        if (code === 0)
                            resolve(null);
                        else
                            reject(new Error('C++ compiler (g++) is not installed. Please install g++ to run C++ code.'));
                    });
                });
            }
            catch (error) {
        ...
        `;
                await (0, promises_1.writeFile)(filePath, wrappedCode);
                process = (0, child_process_1.spawn)('node', [filePath]);
                break;
            case 'python':
                // For Python, add memory tracking without resource module
                const pythonCode = `
import sys
import os
import time
...
${code.split('\n').map(line => '    ' + line).join('\n')}
    ...
    print('Memory Usage: 0.00 MB')  # Memory tracking disabled for Windows compatibility
except Exception as e:
    print(f'Error: {str(e)}', file=sys.stderr)
    sys.exit(1)
`;
                await (0, promises_1.writeFile)(filePath, pythonCode);
                process = (0, child_process_1.spawn)('python', [filePath]);
                break;
            case 'java':
                // Extract class name from code or use default
                const classNameMatch = code.match(/public\s+class\s+(\w+)/);
                const className = classNameMatch ? classNameMatch[1] : 'Main';
                const javaFile = (0, path_1.join)(tempDir, `${className}.java`);
                const javaClass = (0, path_1.join)(tempDir, `${className}.class`);
                // Create a complete Java program with timing and memory tracking
                const javaProgram = code.includes('public class') ? code : `
public class ${className} {
    public static void main(String[] args) {
        ...
        double memoryUsed = (finalMemory - initialMemory) / (1024.0 * 1024.0); // Convert to MB
        ...
        System.out.println("Execution Time: " + String.format("%.3f", executionTime) + " s");
        System.out.println("Memory Usage: " + String.format("%.2f", memoryUsed) + " MB");
    }
}`;
                try {
                    // Write the Java file
                    await (0, promises_1.writeFile)(javaFile, javaProgram);
                    // Compile
                    await new Promise((resolve, reject) => {
                        const javac = (0, child_process_1.spawn)('javac', [javaFile]);
                        let error = '';
                    ...
                    const result = await new Promise((resolve, reject) => {
                        const java = (0, child_process_1.spawn)('java', ['-cp', tempDir, className]);
                        let output = '';
                        let error = '';
                        let executionTime = 0;
                        let memoryUsage = 0;
                        ...
                        java.on('close', (code) => {
                            if (code === 0) {
                                resolve({ output: output.trim(), executionTime, memoryUsage });
                            }
                            else {
                    ...
                    console.log('Java execution output:', finalOutput); // Debug log
                    return {
                        output: finalOutput,
                        error: null,
                    ...
                    try {
                        ...
                        const fileExists = await promises_2.default.access(javaFile).then(() => true).catch(() => false);
                        const classExists = await promises_2.default.access(javaClass).then(() => true).catch(() => false);
                        if (fileExists)
                            await (0, promises_1.unlink)(javaFile);
                        if (classExists)
                            await (0, promises_1.unlink)(javaClass);
                    }
                    catch (e) {
...
int main() {
    try {
        ...
        std::cout << "Execution Time: " << std::fixed << std::setprecision(3) << executionTime << " s" << std::endl;
        std::cout << "Memory Usage: 0.00 MB" << std::endl;  // Memory tracking disabled for Windows compatibility
        ...
        return 1;
                ...
                await (0, promises_1.writeFile)(cppFile, cppCode);
                // Compile with proper flags for Windows
                const cppCompileProcess = (0, child_process_1.spawn)('g++', [cppFile, '-o', filePath, '-std=c++11', '-Wall']);
                let compileError = '';
                await new Promise((resolve, reject) => {
                    ...
                    cppCompileProcess.on('close', (code) => {
                        if (code === 0)
                            ...
                            reject(new Error(`C++ compilation failed: ${compileError}`));
                    });
                });
                // Run the compiled program
                const result = await new Promise((resolve, reject) => {
                    const cppProcess = (0, child_process_1.spawn)(filePath);
                    let output = '';
                    let error = '';
                    let executionTime = 0;
        ...
        process.stdout.on('data', (data) => {
            ...
            const memoryMatch = text.match(/Memory Usage: ([\d.]+) MB/);
            if (memoryMatch) {
                performanceMetrics = `Memory Usage: ${memoryMatch[1]} MB`;
            }
        });
        ...
        return {
            output: output.replace(performanceMetrics, '').trim(), // Remove performance metrics from output
        ...
        try {
            ...
            if (language === 'java') {
                const javaFile = (0, path_1.join)(tempDir, 'Main.java');
                const javaClass = (0, path_1.join)(tempDir, 'Main.class');
                try {
                    await promises_2.default.access(javaFile);
                    await (0, promises_1.unlink)(javaFile);
                }
                catch (e) {
                ...
                try {
                    await promises_2.default.access(javaClass);
                    await (0, promises_1.unlink)(javaClass);
                }
                catch (e) {
                    // File doesn't exist, ignore error
                ...
                const cppFile = `${filePath}.cpp`;
                try {
                    await promises_2.default.access(cppFile);
                    await (0, promises_1.unlink)(cppFile);
                }
                catch (e) {
                    // File doesn't exist, ignore error
                }
